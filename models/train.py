...
from data_augment import CutMix, MixUp
...
parser.add_argument("--criterion", default="ce", help="[ce, mse, hinge, hinge+l2]")
...
parser.add_argument("--mlp-hidden", default=1536, type=int)
...
# pin_memory=True: 将数据保存在固定的内存区域，这样可以加速GPU上的数据传输
...
class Net(pl.LightningModule):
    ...
    def configure_optimizers(self):
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.hparams.lr, betas=(self.hparams.beta1, self.hparams.beta2), weight_decay=self.hparams.weight_decay)  # 优化器：Adam
        self.base_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(self.optimizer, T_max=self.hparams.max_epochs, eta_min=self.hparams.min_lr)    # 学习率调度器：余弦退火
        self.scheduler = warmup_scheduler.GradualWarmupScheduler(self.optimizer, multiplier=1., total_epoch=self.hparams.warmup_epoch, after_scheduler=self.base_scheduler) # 学习率调度器：warmup，target learning rate is reached at total_epoch gradually
...
if __name__ == "__main__":
    ...
    net = Net(args)
    refresh_rate = 1
    trainer = pl.Trainer(precision=args.precision,fast_dev_run=args.dry_run, gpus="1", benchmark=args.benchmark, logger=logger, max_epochs=args.max_epochs, weights_summary="full", progress_bar_refresh_rate=refresh_rate) # weights_summary prints a summary of the weights when training begins.
